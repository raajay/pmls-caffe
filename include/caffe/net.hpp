...
public:
  ...
  explicit Net(const NetParameter &param, const int thread_id,
               const int net_id);
  explicit Net(const string &param_file, const int thread_id, const int net_id);
  virtual ~Net() {}
  ...
  inline vector<Dtype> &blob_loss_weights() { return blob_loss_weights_; }
  ...
  inline void set_table(const int global_id) {
    ...
    outputs_global_table_ =
        petuum::PSTableGroup::GetTableOrDie<Dtype>(global_id_);
  }
