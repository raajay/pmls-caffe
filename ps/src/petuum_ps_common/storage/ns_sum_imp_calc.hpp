...
template <typename V> class NSSumImpCalc : public NSAbstractImpCalc<V> {
public:
  NSSumImpCalc() {}
  virtual ~NSSumImpCalc() {}

  virtual double ApplyIncGetImportance(AbstractStore<V> *store,
                                       int32_t column_id, const void *update);

  virtual double ApplyBatchIncGetImportance(AbstractStore<V> *store,
                                            const int32_t *column_ids,
                                            const void *update_batch,
                                            int32_t num_updates);

  virtual double ApplyDenseBatchIncGetImportance(AbstractStore<V> *store,
                                                 const void *update_batch,
                                                 int32_t index_st,
                                                 int32_t num_updates);
  ...
  virtual double GetDenseAccumImportance(const AbstractStore<V> *store,
                                         const void *update_batch,
                                         int32_t index_st,
                                         int32_t num_updates) const;
};
...
template <typename V>
double NSSumImpCalc<V>::ApplyIncGetImportance(AbstractStore<V> *store,
                                              int32_t column_id,
                                              const void *update) {
...
template <typename V>
double NSSumImpCalc<V>::ApplyBatchIncGetImportance(AbstractStore<V> *store,
                                                   const int32_t *column_ids,
                                                   const void *update_batch,
                                                   int32_t num_updates) {
...
double NSSumImpCalc<V>::ApplyDenseBatchIncGetImportance(
    AbstractStore<V> *store, const void *update_batch, int32_t index_st,
    int32_t num_updates) {
...
template <typename V>
double NSSumImpCalc<V>::GetImportance(int32_t column_id, const void *update,
                                      const void *value) const {
  V typed_update = *(reinterpret_cast<const V *>(update));
  V typed_value = *(reinterpret_cast<const V *>(value));
  double importance = (double(typed_value) == 0)
                          ? double(typed_update)
...
template <typename V>
double NSSumImpCalc<V>::GetImportance(int32_t column_id,
...
template <typename V>
double NSSumImpCalc<V>::GetAccumImportance(const int32_t *column_ids,
                                           const void *update_batch,
                                           int32_t num_updates) const {
  const V *update_array = reinterpret_cast<const V *>(update_batch);
  double accum_importance = 0;
...
template <typename V>
double NSSumImpCalc<V>::GetDenseAccumImportance(const AbstractStore<V> *store,
                                                const void *update_batch,
                                                int32_t index_st,
                                                int32_t num_updates) const {
  const V *update_array = reinterpret_cast<const V *>(update_batch);
  ...
  for (int i = 0; i < num_updates; ++i) {
    ...
    double importance = (double(val) == 0)
                            ? double(update_array[i])
                            : double(update_array[i]) / double(val);
    accum_importance += std::abs(importance);
