...
template <typename V> class VectorStoreFloat16 : public AbstractStore<V> {
public:
  VectorStoreFloat16();
  ~VectorStoreFloat16();
...
template <typename V> VectorStoreFloat16<V>::VectorStoreFloat16() {}
...
template <typename V>
VectorStoreFloat16<V> &VectorStoreFloat16<V>::
operator=(const VectorStoreFloat16<V> &other) {
  data_ = other.data_;
  return *this;
}

template <typename V> void VectorStoreFloat16<V>::Init(size_t capacity) {
  data_.resize(capacity);
  memset(data_.data(), 0, capacity * sizeof(V));
}

template <typename V> size_t VectorStoreFloat16<V>::SerializedSize() const {
...
template <typename V> V VectorStoreFloat16<V>::Get(int32_t col_id) const {
  V v = data_[col_id];
  return v;
}

template <typename V> void VectorStoreFloat16<V>::Inc(int32_t col_id, V delta) {
  data_[col_id] += delta;
}

template <typename V> V *VectorStoreFloat16<V>::GetPtr(int32_t col_id) {
  return data_.data() + col_id;
}

template <typename V> size_t VectorStoreFloat16<V>::get_capacity() const {
  return data_.size();
}

template <typename V> const void VectorStoreFloat16<V>::Copy(void *to) const {
  std::vector<V> *vec = reinterpret_cast<std::vector<V> *>(to);
  memcpy(vec->data(), data_.data(), data_.size() * sizeof(V));
}

template <typename V> const void *VectorStoreFloat16<V>::GetDataPtr() const {
  return &data_;
}
}
