...
template <typename V> class VectorStore : public AbstractStore<V> {
public:
  VectorStore();
  ~VectorStore();
...
template <typename V> VectorStore<V>::VectorStore() {}
...
template <typename V> void VectorStore<V>::Init(size_t capacity) {
  data_.resize(capacity);
  memset(data_.data(), 0, capacity * sizeof(V));
}

template <typename V> size_t VectorStore<V>::SerializedSize() const {
  return data_.size() * sizeof(V);
}

template <typename V> size_t VectorStore<V>::Serialize(void *bytes) const {
...
template <typename V> V VectorStore<V>::Get(int32_t col_id) const {
  V v = data_[col_id];
  return v;
}

template <typename V> void VectorStore<V>::Inc(int32_t col_id, V delta) {
  data_[col_id] += delta;
}

template <typename V> V *VectorStore<V>::GetPtr(int32_t col_id) {
...
template <typename V> size_t VectorStore<V>::get_capacity() const {
  return data_.size();
}

template <typename V> const void VectorStore<V>::CopyToVector(void *to) const {
  auto &vec = *(reinterpret_cast<std::vector<V> *>(to));
  memcpy(vec.data(), data_.data(), data_.size() * sizeof(V));
}

template <typename V> const void VectorStore<V>::CopyToMem(void *to) const {
  memcpy(to, data_.data(), data_.size() * sizeof(V));
}

template <typename V> const void *VectorStore<V>::GetDataPtr() const {
  return &data_;
}
}
