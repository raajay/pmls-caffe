...
template <typename V> class SortedVectorMapStore : public AbstractStore<V> {
public:
  ...
  public:
    ConstIterator(Entry<V> *begin, Entry<V> *end)
        : entry_ptr_(begin), end_ptr_(end) {}

    ~ConstIterator() {}

    ConstIterator(const ConstIterator &other)
        : entry_ptr_(other.entry_ptr_), end_ptr_(other.end_ptr_) {}
...
template <typename V> const size_t SortedVectorMapStore<V>::kBlockSize = 64;

template <typename V>
SortedVectorMapStore<V>::SortedVectorMapStore()
    : num_entries_(0), capacity_(0) {}

template <typename V> SortedVectorMapStore<V>::~SortedVectorMapStore() {}

template <typename V>
SortedVectorMapStore<V>::SortedVectorMapStore(
    const SortedVectorMapStore<V> &other)
    : entries_(new Entry<V>[other.capacity_]), num_entries_(other.num_entries_),
      capacity_(other.capacity_) {
  memcpy(entries_.get(), other.entries_.get(), num_entries_ * sizeof(Entry<V>));
}

template <typename V>
SortedVectorMapStore<V> &SortedVectorMapStore<V>::
operator=(const SortedVectorMapStore<V> &other) {
...
template <typename V> void SortedVectorMapStore<V>::Init(size_t capacity) {
  entries_.reset(new Entry<V>[capacity]);
  capacity_ = capacity;
  num_entries_ = 0;
}

template <typename V> size_t SortedVectorMapStore<V>::SerializedSize() const {
...
template <typename V> V SortedVectorMapStore<V>::Get(int32_t key) const {
...
template <typename V> void SortedVectorMapStore<V>::Inc(int32_t key, V delta) {
  size_t new_size = 0;
...
const void SortedVectorMapStore<V>::CopyToVector(void *to) const {
  std::vector<Entry<V>> *vec = reinterpret_cast<std::vector<Entry<V>> *>(to);
  vec->resize(num_entries_);
  memcpy(vec->data(), entries_.get(), num_entries_ * sizeof(Entry<V>));
}

template <typename V> size_t SortedVectorMapStore<V>::num_entries() const {
  return num_entries_;
}

template <typename V> size_t SortedVectorMapStore<V>::capacity() const {
  return capacity_;
}

template <typename V>
typename SortedVectorMapStore<V>::ConstIterator
SortedVectorMapStore<V>::CBegin() {
...
template <typename V>
bool SortedVectorMapStore<V>::ShouldCompact(size_t capacity,
                                            size_t num_entries) {
...
template <typename V>
typename SortedVectorMapStore<V>::ConstIterator
    SortedVectorMapStore<V>::ConstIterator::
    operator++() {
  entry_ptr_ += 1;
}

template <typename V> bool SortedVectorMapStore<V>::ConstIterator::is_end() {
