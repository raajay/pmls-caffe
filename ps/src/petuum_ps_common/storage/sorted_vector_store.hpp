...
template <typename V> class SortedVectorStore : public AbstractStore<V> {
public:
  SortedVectorStore();
  ~SortedVectorStore();
...
template <typename V> const size_t SortedVectorStore<V>::kBlockSize = 64;

template <typename V>
SortedVectorStore<V>::SortedVectorStore()
    : num_entries_(0), capacity_(0) {}

template <typename V> SortedVectorStore<V>::~SortedVectorStore() {}
...
template <typename V>
SortedVectorStore<V> &SortedVectorStore<V>::
operator=(const SortedVectorStore<V> &other) {
...
template <typename V> void SortedVectorStore<V>::Init(size_t capacity) {
  entries_.reset(new Entry<V>[capacity]);
  capacity_ = capacity;
  num_entries_ = 0;
}

template <typename V> size_t SortedVectorStore<V>::SerializedSize() const {
...
template <typename V> V SortedVectorStore<V>::Get(int32_t key) const {
...
template <typename V> void SortedVectorStore<V>::Inc(int32_t key, V delta) {
  size_t new_size = 0;
  bool suc = Inc(key, delta, &new_size);
  if (!suc) {
    LOG(FATAL) << "Inc failed"
               << " capacity = " << capacity_ << " new size = " << new_size;
  }
}

template <typename V> const void SortedVectorStore<V>::Copy(void *to) const {
...
template <typename V> size_t SortedVectorStore<V>::num_entries() const {
  return num_entries_;
}

template <typename V> size_t SortedVectorStore<V>::capacity() const {
  return capacity_;
}
