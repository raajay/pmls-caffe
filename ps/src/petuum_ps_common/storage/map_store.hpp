...
template <typename V> class MapStore : public AbstractStore<V> {
public:
  MapStore() {}
  ~MapStore() {}

  MapStore(const MapStore<V> &other) : data_(other.data_) {}
...
template <typename V> V MapStore<V>::Get(int32_t col_id) const {
...
template <typename V> void MapStore<V>::Inc(int32_t col_id, V delta) {
  V &val = data_[col_id];
  val += delta;
  if (val == V(0))
    data_.erase(col_id);
...
template <typename V> const void MapStore<V>::CopyToVector(void *to) const {
...
template <typename V> size_t MapStore<V>::num_entries() const {
  return data_.size();
}

template <typename V> size_t MapStore<V>::SerializedSize() const {
  return data_.size() * (sizeof(int32_t) + sizeof(V));
}

template <typename V> size_t MapStore<V>::Serialize(void *bytes) const {
  void *data_ptr = bytes;
