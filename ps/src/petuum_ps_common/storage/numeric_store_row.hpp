...
AbstractRow *NumericStoreRow<StoreType, V, ImpCalc>::Clone() const {
  ...
  NumericStoreRow<StoreType, V, ImpCalc> *new_row =
      new NumericStoreRow<StoreType, V, ImpCalc>();
...
template <template <typename> class StoreType, typename V,
          template <typename> class ImpCalc>
void
NumericStoreRow<StoreType, V, ImpCalc>::ApplyIncUnsafe(int32_t column_id,
                                                       const void *update) {
...
template <template <typename> class StoreType, typename V,
          template <typename> class ImpCalc>
double
NumericStoreRow<StoreType, V, ImpCalc>::ApplyDenseBatchIncUnsafeGetImportance(
    const void *update_batch, int32_t index_st, int32_t num_updates) {
  return imp_cal_.ApplyDenseBatchIncGetImportance(&store_, update_batch,
                                                  index_st, num_updates);
}

template <template <typename> class StoreType, typename V,
          template <typename> class ImpCalc>
double NumericStoreRow<StoreType, V, ImpCalc>::ApplyIncGetImportance(
    int32_t column_id, const void *update) {
  std::unique_lock<std::mutex> lock(mtx_);
...
double NumericStoreRow<StoreType, V, ImpCalc>::GetDenseAccumImportance(
  ...
  return imp_cal_.GetDenseAccumImportance(&store_, update_batch, index_st,
                                          num_updates);
}
...
void NumericStoreRow<StoreType, V, ImpCalc>::AddUpdates(
    int32_t column_id, void *update1, const void *update2) const {
...
template <template <typename> class StoreType, typename V,
          template <typename> class ImpCalc>
void NumericStoreRow<StoreType, V, ImpCalc>::InitUpdate(int32_t column_id,
                                                        void *zero) const {
...
template <template <typename> class StoreType, typename V,
          template <typename> class ImpCalc>
bool NumericStoreRow<StoreType, V, ImpCalc>::CheckZeroUpdate(
