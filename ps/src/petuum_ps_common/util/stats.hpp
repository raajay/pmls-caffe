...
#define STATS_BG_ACCUM_WAITS_ON_ACK_IDLE() Stats::BgAccumWaitsOnAckIdle()

#define STATS_BG_ACCUM_WAITS_ON_ACK_CLOCK() Stats::BgAccumWaitsOnAckClock()

#define STATS_BG_ACCUM_NUM_OPLOG_METAS_READ(table_id, num_oplog_metas_read,    \
                                            num_new_oplog_metas)               \
  Stats::BgAccumNumOpLogMetasRead(table_id, num_oplog_metas_read,              \
                                  num_new_oplog_metas)
...
#define STATS_SERVER_IDLE_INVOKE_INC_ONE() Stats::ServerIdleInvokeIncOne()

#define STATS_SERVER_IDLE_SEND_INC_ONE() Stats::ServerIdleSendIncOne()

#define STATS_SERVER_ACCUM_IDLE_SEND_BEGIN() Stats::ServerAccumIdleSendBegin()

#define STATS_SERVER_ACCUM_IDLE_SEND_END() Stats::ServerAccumIdleSendEnd()
...
#define STATS_PRINT() Stats::PrintStats()
...
struct BgThreadStats {
  ...
  std::unordered_map<int32_t, std::vector<double>> table_accum_importance;

  std::unordered_map<int32_t, std::vector<size_t>> table_accum_num_rows_sent;
  ...
  std::unordered_map<int32_t, OpLogReadStats> table_oplog_read_stats;
  ...
  BgThreadStats()
      : accum_clock_end_oplog_serialize_sec(0.0),
        ...
        accum_idle_send_bytes_mb(0.0), accum_handle_append_oplog_sec(0),
        num_row_oplog_created(0), num_row_oplog_recycled(0),
        accum_num_waits_on_ack_idle(1, 0), accum_num_waits_on_ack_clock(1, 0) {}
};
...
struct ServerLogicStats {
  ...
  ServerLogicStats()
      : num_idle_send_check(0), num_idle_send_check_rejected(0),
...
struct ServerThreadStats {
  ...
  std::map<int32_t, ServerLogicStats> table_logic_stats;
  ...
  ServerThreadStats()
      : accum_apply_oplog_sec(0.0), accum_push_row_sec(0.0),
        ...
        per_clock_accum_dup_rows_sent(1, 0), clock_num(0),
        accum_num_oplog_msg_recv(0), accum_num_push_row_msg_send(0),
        accum_num_idle_invoke(0), accum_num_idle_send(0),
        accum_idle_send_sec(0.0), accum_idle_send_bytes_mb(0.0),
        accum_num_waits_on_ack_idle(1, 0), accum_num_waits_on_ack_clock(1, 0) {}
};
...
public:
  ...
  static void BgAccumImportance(int32_t table_id, MetaRowOpLog *meta_row_oplog,
                                bool row_sent);
  // This is called before the to-be-sent oplogs are added to the buffer, by
  // each thread
  ...
  static void BgAccumNumOpLogMetasRead(int32_t table_id,
                                       size_t num_oplog_metas_read,
                                       size_t num_new_oplog_metas);

  static void BgAccumTableOpLogSent(int32_t table_id, int32_t row_id,
                                    size_t count);
  static void BgAccumTableRowRecved(int32_t table_id, int32_t row_id,
                                    size_t count);
  ...
  static void ServerAccumImportance(int32_t table_id, double importance,
                                    bool row_sent);

  static void ServerAccumWaitsOnAckIdle();
  static void ServerAccumWaitsOnAckClock();

  static void ServerAccumCheck(int32_t table_id, bool permitted,
...
private:
  ...
  static std::unordered_map<int32_t, std::vector<double>>
      bg_table_accum_importance_;
  static std::unordered_map<int32_t, std::vector<size_t>>
      bg_table_accum_num_rows_sent_;
  ...
  static std::map<int32_t, std::map<int32_t, size_t>> bg_table_oplog_send_freq_;

  static std::map<int32_t, std::map<int32_t, size_t>> bg_table_row_recv_freq_;
  ...
  static std::unordered_map<int32_t, std::vector<double>>
      server_table_accum_importance_;
  static std::unordered_map<int32_t, std::vector<size_t>>
      server_table_accum_num_rows_sent_;
