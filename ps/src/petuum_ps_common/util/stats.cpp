...
std::unordered_map<int32_t, std::vector<double>>
    Stats::bg_table_accum_importance_;
std::unordered_map<int32_t, std::vector<size_t>>
    Stats::bg_table_accum_num_rows_sent_;
...
std::map<int32_t, std::map<int32_t, size_t>> Stats::bg_table_oplog_send_freq_;

std::map<int32_t, std::map<int32_t, size_t>> Stats::bg_table_row_recv_freq_;
...
std::unordered_map<int32_t, std::vector<double>>
    Stats::server_table_accum_importance_;
std::unordered_map<int32_t, std::vector<size_t>>
    Stats::server_table_accum_num_rows_sent_;
...
void Stats::DeregisterBgThread() {
  ...
  for (const auto &table_pair : stats.table_accum_importance) {
    ...
    if (table_iter == bg_table_accum_importance_.end()) {
      bg_table_accum_importance_.insert(std::make_pair(
          table_id, std::vector<double>(table_pair.second.size(), 0.0)));
  ...
  for (const auto &table_pair : stats.table_accum_num_rows_sent) {
    ...
    if (table_iter == bg_table_accum_num_rows_sent_.end()) {
      bg_table_accum_num_rows_sent_.insert(std::make_pair(
          table_id, std::vector<size_t>(table_pair.second.size(), 0)));
  ...
  if (bg_accum_num_waits_on_ack_idle_.size() == 0) {
    ...
    for (int i = 0; i < stats.accum_num_waits_on_ack_idle.size(); ++i) {
      bg_accum_num_waits_on_ack_idle_[i] = stats.accum_num_waits_on_ack_idle[i];
    }
  }
  ...
  for (auto &table_stats : stats.table_oplog_read_stats) {
    ...
    auto table_iter = bg_oplog_read_stats_.find(table_id);
    if (table_iter == bg_oplog_read_stats_.end()) {
      bg_oplog_read_stats_.insert(std::make_pair(table_id, table_stats.second));
      continue;
    }
...
void Stats::DeregisterServerThread() {
  ...
  for (int i = 0; i < stats.per_clock_accum_dup_rows_sent.size(); ++i) {
  ...
  for (const auto &table_pair : stats.table_accum_importance) {
    ...
    if (table_iter == server_table_accum_importance_.end()) {
      server_table_accum_importance_.insert(std::make_pair(
          table_id, std::vector<double>(table_pair.second.size(), 0.0)));
  ...
  for (const auto &table_pair : stats.table_accum_num_rows_sent) {
    ...
    if (table_iter == server_table_accum_num_rows_sent_.end()) {
      server_table_accum_num_rows_sent_.insert(std::make_pair(
          table_id, std::vector<size_t>(table_pair.second.size(), 0)));
...
void Stats::BgAccumImportance(int32_t table_id, double importance,
  ...
  auto table_iter = stats.table_accum_importance.find(table_id);
  if (table_iter == stats.table_accum_importance.end()) {
    stats.table_accum_importance.insert(
        std::make_pair(table_id, std::vector<double>(0)));
  ...
  auto num_rows_iter = stats.table_accum_num_rows_sent.find(table_id);
  if (num_rows_iter == stats.table_accum_num_rows_sent.end()) {
    stats.table_accum_num_rows_sent.insert(
        std::make_pair(table_id, std::vector<size_t>(0)));
...
void Stats::BgAccumNumOpLogMetasRead(int32_t table_id,
                                     size_t num_oplog_metas_read,
                                     size_t num_new_oplog_metas) {
  BgThreadStats &stats = *bg_thread_stats_;
  auto table_iter = stats.table_oplog_read_stats.find(table_id);
...
void Stats::BgAccumTableOpLogSent(int32_t table_id, int32_t row_id,
                                  size_t count) {
  BgThreadStats &stats = *bg_thread_stats_;
  stats.table_oplog_send_freq[table_id][row_id] += count;
}

void Stats::BgAccumTableRowRecved(int32_t table_id, int32_t row_id,
                                  size_t count) {
  BgThreadStats &stats = *bg_thread_stats_;
...
void Stats::ServerAddPerClockAccumDupRowsSent(size_t rows_sent) {
  ServerThreadStats &stats = *server_thread_stats_;
  stats.per_clock_accum_dup_rows_sent[stats.clock_num] += rows_sent;
}
...
void Stats::ServerAccumImportance(int32_t table_id, double importance,
                                  bool row_sent) {
  ServerThreadStats &stats = *server_thread_stats_;
...
void Stats::ServerAccumCheck(int32_t table_id, bool permitted,
                             size_t logic_info_size) {
...
void Stats::PrintStats() {
  ...
  yaml_out << YAML::Key << "bg_accum_idle_send_sec" << YAML::Value;
  YamlPrintSequence(&yaml_out, bg_accum_idle_send_sec_);

  yaml_out << YAML::Key << "bg_accum_idle_send_bytes_mb" << YAML::Value;
  ...
  yaml_out << YAML::Key << "bg_table_accum_importance" << YAML::Value
           << YAML::BeginMap;

  for (const auto &table_pair : bg_table_accum_importance_) {
    int32_t table_id = table_pair.first;
    yaml_out << YAML::Key << table_id << YAML::Value;
    YamlPrintSequence(&yaml_out, table_pair.second);
  }

  yaml_out << YAML::EndMap;

  yaml_out << YAML::Key << "bg_accum_num_rows_sent" << YAML::Value
           << YAML::BeginMap;
  ...
  yaml_out << YAML::Key << "bg_accum_num_waits_on_ack_idle" << YAML::Value;
  YamlPrintSequence(&yaml_out, bg_accum_num_waits_on_ack_idle_);

  yaml_out << YAML::Key << "bg_accum_num_waits_on_ack_clock" << YAML::Value;
  YamlPrintSequence(&yaml_out, bg_accum_num_waits_on_ack_clock_);

  yaml_out << YAML::Key << "bg_table_oplog_read_stats" << YAML::Value;
  yaml_out << YAML::BeginMap;

  for (auto &table_stats : bg_oplog_read_stats_) {
    yaml_out << YAML::Key << table_stats.first << YAML::Value << YAML::BeginMap
             << YAML::Key << "accum_num_oplog_index_reads" << YAML::Value
             << table_stats.second.accum_num_oplog_index_reads << YAML::Key
             << "accum_num_oplog_metas_read" << YAML::Value
             << table_stats.second.accum_num_oplog_metas_read << YAML::Key
             << "accum_num_new_oplog_meta" << YAML::Value
             << table_stats.second.accum_num_new_oplog_meta << YAML::EndMap;
  }
  yaml_out << YAML::EndMap;
  yaml_out << YAML::EndMap;
  ...
  yaml_out << YAML::BeginMap << YAML::Comment("ServerThread Stats") << YAML::Key
           ...
           << "server_accum_total_serialize_push_row_sec" << YAML::Value
  ...
  yaml_out << YAML::Key << "server_accum_num_idle_invoke" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_num_idle_invoke_);

  yaml_out << YAML::Key << "server_accum_num_idle_send" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_num_idle_send_);

  yaml_out << YAML::Key << "server_accum_idle_send_sec" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_idle_send_sec_);

  yaml_out << YAML::Key << "server_accum_idle_send_bytes_mb" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_idle_send_bytes_mb_);

  yaml_out << YAML::Key << "server_table_accum_importance" << YAML::Value
           << YAML::BeginMap;
  ...
  yaml_out << YAML::Key << "server_accum_num_rows_sent" << YAML::Value
           << YAML::BeginMap;
  ...
  yaml_out << YAML::Key << "server_accum_num_waits_on_ack_idle" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_num_waits_on_ack_idle_);

  yaml_out << YAML::Key << "server_accum_num_waits_on_ack_clock" << YAML::Value;
  YamlPrintSequence(&yaml_out, server_accum_num_waits_on_ack_clock_);

  yaml_out << YAML::Key << "server_table_logic" << YAML::Value
           << YAML::BeginMap;
  ...
  for (auto &table_logic_stats : server_table_logic_stats_) {
    ...
    yaml_out << YAML::Key << "num_idle_send_check" << YAML::Value
             << table_logic_stats.second.num_idle_send_check;
    yaml_out << YAML::Key << "num_idle_send_check_rejected" << YAML::Value
             << table_logic_stats.second.num_idle_send_check_rejected;
    yaml_out << YAML::Key << "accum_logic_info_size" << YAML::Value
             << table_logic_stats.second.accum_logic_info_size;
    yaml_out << YAML::EndMap;
  }
  ...
  std::fstream of_stream(stats_path_,
  ...
  std::fstream oplog_send_of(bg_oplog_send_freq_ss.str(),
                             std::ios_base::out | std::ios_base::trunc);
  ...
  std::fstream row_recv_of(bg_row_recv_freq_ss.str(),
                           std::ios_base::out | std::ios_base::trunc);
