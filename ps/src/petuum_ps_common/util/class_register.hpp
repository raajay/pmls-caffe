...
namespace petuum {
template <typename BaseClass> class ClassRegistry {
public:
  typedef BaseClass *(*CreateFunc)();
  ClassRegistry() : default_creator_(0){};
  ~ClassRegistry(){};

  void SetDefaultCreator(CreateFunc creator) { default_creator_ = creator; }
...
template <typename BaseClass, typename ImplClass> BaseClass *CreateObj() {
