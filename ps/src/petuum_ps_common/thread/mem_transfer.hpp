...
class MemTransfer {
public:
  // Transfer memory of msg (of type MemBlock) ownership to thread recv_id, who
  // is
  // responsible for destroying the received MemBlock via
  // DestroyTransferredMem().
  // Return true if the memory is transferred without copying otherwise return
  // false.
  // MemBlock is released from msg regardless wether or not the memory is
  // copied.
  static bool TransferMem(CommBus *comm_bus, int32_t recv_id,
    ...
    if (comm_bus->IsLocalEntity(recv_id)) {
      ...
      size_t sent_size = comm_bus->SendInProc(
          recv_id, mem_transfer_msg.get_mem(), mem_transfer_msg.get_size());
      CHECK_EQ(sent_size, mem_transfer_msg.get_size());
      return true;
    } else {
      size_t sent_size =
          comm_bus->SendInterProc(recv_id, msg->get_mem(), msg->get_size());
