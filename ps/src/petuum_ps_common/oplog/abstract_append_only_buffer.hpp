...
public:
  AbstractAppendOnlyBuffer(int32_t thread_id, size_t capacity,
                           size_t update_size)
      : thread_id_(thread_id), capacity_(capacity), size_(0),
        update_size_(update_size), buff_(new uint8_t[capacity]) {}

  virtual ~AbstractAppendOnlyBuffer() {}

  int32_t get_thread_id() const { return thread_id_; }
  ...
  virtual bool DenseBatchInc(int32_t row_id, const void *updates,
