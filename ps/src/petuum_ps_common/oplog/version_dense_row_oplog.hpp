...
public:
  VersionDenseRowOpLog(InitUpdateFunc InitUpdate,
                       ...
                       size_t row_size)
      : AbstractRowOpLog(update_size), row_size_(row_size),
        oplogs_(new uint8_t[update_size * row_size]), InitUpdate_(InitUpdate),
        CheckZeroUpdate_(CheckZeroUpdate), version_(0), end_of_version_(false) {
    CHECK(row_size > 0);
    Reset();
  }
  ...
  void Reset() { memset(oplogs_.get(), 0, update_size_ * row_size_); }

  void *Find(int32_t col_id) { return oplogs_.get() + col_id * update_size_; }

  const void *FindConst(int32_t col_id) const {
    return oplogs_.get() + col_id * update_size_;
  }

  void *FindCreate(int32_t col_id) { return Find(col_id); }
  ...
  const void *NextConst(int32_t *column_id) const {
    const uint8_t *update =
        reinterpret_cast<const uint8_t *>(FindConst(iter_col_id_));
    while (CheckZeroUpdate_(update) && iter_col_id_ < row_size_) {
      update += update_size_;
      ++iter_col_id_;
    }
  ...
  size_t GetSize() const { return row_size_; }
  ...
  size_t GetSparseSerializedSize() {
    return sizeof(int32_t) + sizeof(int32_t) * num_nonzeros_ +
           update_size_ * num_nonzeros_ + sizeof(uint64_t) + sizeof(bool);
  }

  size_t GetDenseSerializedSize() {
    return row_size_ * update_size_ + sizeof(uint64_t) + sizeof(bool);
  }
