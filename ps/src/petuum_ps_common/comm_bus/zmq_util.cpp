...
int32_t ZMQUtil::ZmqID2EntityID(int32_t zmq_id) { return (zmq_id >> 4); }

void ZMQUtil::ZMQSetSockOpt(zmq::socket_t *sock, int option, const void *optval,
                            size_t optval_size) {
...
void ZMQUtil::ZMQConnectSend(zmq::socket_t *sock,
                             const std::string &connect_addr, int32_t zmq_id,
...
size_t ZMQUtil::ZMQSend(zmq::socket_t *sock, const void *data, size_t len,
  ...
  try {
    ...
    default:
      LOG(FATAL) << e.what()
                 << " data = " << *(reinterpret_cast<const int32_t *>(data))
...
size_t ZMQUtil::ZMQSend(zmq::socket_t *sock, int32_t zmq_id, const void *data,
                        size_t len, int flag) {
  size_t zid_sent_size =
      ZMQSend(sock, &zmq_id, sizeof(zmq_id), flag | ZMQ_SNDMORE);
