...
public:
  ServerThread(int32_t my_id, pthread_barrier_t *init_barrier)
      : my_id_(my_id), bg_worker_ids_(GlobalContext::get_num_clients()),
        num_shutdown_bgs_(0), comm_bus_(GlobalContext::comm_bus),
        init_barrier_(init_barrier), msg_tracker_(kMaxPendingMsgs),
        pending_clock_push_row_(false), pending_shut_down_(false),
...
protected:
  ...
  virtual void HandleBgServerPushRowAck(int32_t bg_id, uint64_t ack_seq);

  virtual long ServerIdleWork();
  virtual long ResetServerIdleMilli();

  virtual void SendOpLogAckMsg(int32_t bg_id, uint32_t version, uint64_t seq);
