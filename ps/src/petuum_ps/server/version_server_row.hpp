...
public:
  VersionServerRow() : ServerRow(), version_(1) {}

  explicit VersionServerRow(AbstractRow *row_data)
      : ServerRow(row_data), version_(1) {}

  VersionServerRow(VersionServerRow &&other)
      : ServerRow(std::move(other)), version_(other.version_) {}
  ...
  void ApplyBatchIncAccumImportance(const int32_t *column_ids,
                                    ...
                                    int32_t num_updates) {
    ServerRow::ApplyBatchIncAccumImportance(column_ids, update_batch,
                                            num_updates);
    version_++;
  }
