...
void Server::ApplyOpLogUpdateVersion(const void *oplog, size_t oplog_size,
  ...
  while (updates != 0) {
    ...
    updates = oplog_reader.Next(&table_id, &row_id, &column_ids, &num_updates,
                                &started_new_table);
...
size_t Server::CreateSendServerPushRowMsgs(PushMsgSendFunc PushMsgSend,
  ...
  for (client_id = 0; client_id < GlobalContext::get_num_clients();
    ...
    buffs.insert(std::make_pair(
        client_id, RecordBuff(msg->get_data(), push_row_msg_data_size_)));
  }
  ...
  for (auto table_iter = tables_.begin(); table_iter != tables_.end();
    ...
    for (client_id = 0; client_id < GlobalContext::get_num_clients();
      ...
      if (table_id_ptr == 0) {
        int32_t bg_id =
            GlobalContext::get_bg_thread_id(client_id, comm_channel_idx);
    ...
    bool pack_suc = server_table.AppendTableToBuffs(
        0, &buffs, &failed_client_id, false, &num_clients);
    ...
    while (!pack_suc) {
      ...
      int32_t bg_id =
          GlobalContext::get_bg_thread_id(failed_client_id, comm_channel_idx);
      ...
      *table_id_ptr = table_id;
      pack_suc = server_table.AppendTableToBuffs(
          failed_client_id, &buffs, &failed_client_id, true, &num_clients);
    }
...
size_t Server::CreateSendServerPushRowMsgsPartial(PushMsgSendFunc PushMsgSend) {
  boost::unordered_map<int32_t, RecordBuff> buffs;
  boost::unordered_map<int32_t, ServerPushRowMsg *> msg_map;
  boost::unordered_map<int32_t, size_t> client_buff_size;
  boost::unordered_map<int32_t, std::vector<std::pair<int32_t, ServerRow *>>>
      table_rows_to_send;
  ...
  for (auto &table_pair : tables_) {
    ...
    table_rows_to_send.insert(std::make_pair(
        table_id, std::vector<std::pair<int32_t, ServerRow *>>()));

    table_pair.second.GetPartialTableToSend(&(table_rows_to_send[table_id]),
                                            &client_buff_size);
    if (table_rows_to_send[table_id].size() > 0)
      has_to_send = true;
  }
  ...
  for (auto buff_size_iter = client_buff_size.begin();
    ...
    if (buff_size_iter->second > 0)
      buff_size_iter->second +=
          (sizeof(int32_t) + sizeof(int32_t)) * num_tables;
  }
  ...
  for (auto buff_size_iter = client_buff_size.begin();
    ...
    msg_map[client_id] = msg;
    buffs.insert(
        std::make_pair(client_id, RecordBuff(msg->get_data(), buff_size)));
  }
  ...
  for (auto table_iter = tables_.begin(); table_iter != tables_.end();
    ...
    server_table.AppendRowsToBuffsPartial(&buffs, table_rows_to_send[table_id]);
  ...
  for (auto msg_iter = msg_map.begin(); msg_iter != msg_map.end(); msg_iter++) {
...
bool Server::AccumedOpLogSinceLastPush() { return accum_oplog_count_ > 0; }

void Server::RowSent(int32_t table_id, int32_t row_id, ServerRow *row,
                     size_t num_clients) {
  auto table_iter = tables_.find(table_id);
