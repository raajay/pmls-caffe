...
void SSPPushServerThread::SendServerPushRowMsg(int32_t bg_id,
                                               ServerPushRowMsg *msg,
...
void SSPPushServerThread::HandleBgServerPushRowAck(int32_t bg_id,
                                                   uint64_t ack_seq) {
  // LOG(INFO) << "acked " << bg_id << " " << ack_seq;
  msg_tracker_.RecvAck(bg_id, ack_seq);
  if (pending_clock_push_row_ && msg_tracker_.CheckSendAll()) {
