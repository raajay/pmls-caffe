...
void AdaRevisionServerTableLogic::ApplyRowOpLog(
    int32_t row_id, const int32_t *col_ids, const void *updates,
    int32_t num_updates, ServerRow *server_row, uint64_t row_version,
    bool end_of_version) {
  ...
  if (row_version == 0) {
    ...
    for (int i = 0; i < num_updates; ++i) {
      ...
      CHECK(delta == delta)
          << "u = " << update << " eta = " << eta << " delta = " << delta
          << " z_max = " << adarev_row.z_max_[i] << " z_ = " << adarev_row.z_[i]
          << " r = " << row_id << " i = " << i << " g_bck = " << g_bck
          << " accum_gradients = " << adarev_row.accum_gradients_[i];
    }
  } else {
    ...
    auto &old_accum_grad_client_count = old_accum_grad_iter->second.second;
...
void AdaRevisionServerTableLogic::ServerRowSent(int32_t row_id,
                                                uint64_t version,
                                                size_t num_clients) {
  CHECK(num_clients > 0);
  auto adarev_iter = adarevision_info_.find(row_id);
  ...
  old_accum_gradients_.insert(
      std::make_pair(std::make_pair(row_id, version),
...
bool AdaRevisionServerTableLogic::AllowSend() {
  ...
  STATS_SERVER_ACCUM_CHECK(0, (info_size < FLAGS_old_grad_upper_bound),
                           info_size);
