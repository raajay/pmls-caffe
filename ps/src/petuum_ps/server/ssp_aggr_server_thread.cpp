...
namespace petuum {
void SSPAggrServerThread::SetWaitMsg() { WaitMsg_ = WaitMsgTimeOut; }
...
long SSPAggrServerThread::ResetServerIdleMilliNoEarlyComm() { return -1; }
...
void SSPAggrServerThread::ServerPushRow() {
  ...
  if (row_send_milli_sec_ > 1) {
    ...
    left_over_send_milli_sec =
        std::max<double>(0, row_send_milli_sec_ - send_elapsed_milli);
  }
...
void SSPAggrServerThread::AdjustSuppressionLevel(int32_t bg_id,
                                                 int32_t bg_clock) {
