...
ClientTable::ClientTable(int32_t table_id, const ClientTableConfig &config)
  ...
  case BoundedDense: {
    ...
    if (GlobalContext::get_consistency_model() == SSP) {
      ...
      StorageCreateClientRow =
          std::bind(&ClientTable::CreateClientRow, this, std::placeholders::_1);
    } else {
  ...
  case SSPPush: {
    if (config.oplog_type == Sparse || config.oplog_type == Dense) {
      consistency_controller_ = new SSPPushConsistencyController(
          config.table_info, table_id, *process_storage_, *oplog_, sample_row_,
          thread_cache_, oplog_index_, row_oplog_type_);
    } else if (config.oplog_type == AppendOnly) {
      consistency_controller_ = new SSPPushAppendOnlyConsistencyController(
          config.table_info, table_id, *process_storage_, *oplog_, sample_row_,
          thread_cache_, oplog_index_, row_oplog_type_);
    } else {
      LOG(FATAL) << "Unknown oplog type = " << config.oplog_type;
  ...
  case SSPAggr: {
    ...
    if (update_sort_policy == RelativeMagnitude ||
        update_sort_policy == FIFO_N_ReMag || update_sort_policy == Random) {
#else
    if (update_sort_policy == RelativeMagnitude ||
        update_sort_policy == FIFO_N_ReMag) {
#endif
      consistency_controller_ = new SSPAggrValueConsistencyController(
          config.table_info, table_id, *process_storage_, *oplog_, sample_row_,
          thread_cache_, oplog_index_, row_oplog_type_);
    } else {
      consistency_controller_ = new SSPAggrConsistencyController(
          config.table_info, table_id, *process_storage_, *oplog_, sample_row_,
          thread_cache_, oplog_index_, row_oplog_type_);
    }
  } break;
