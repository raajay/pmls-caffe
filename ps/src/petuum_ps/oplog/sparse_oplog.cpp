...
SparseOpLog::SparseOpLog(int capacity, const AbstractRow *sample_row,
                         ...
                         int32_t row_oplog_type, bool version_maintain)
    : update_size_(sample_row->get_update_size()),
      locks_(GlobalContext::GetLockPoolSize(capacity)),
      oplog_map_(capacity * kCuckooExpansionFactor), sample_row_(sample_row),
...
int32_t SparseOpLog::Inc(int32_t row_id, int32_t column_id, const void *delta) {
  ...
  if (!oplog_map_.find(row_id, row_oplog)) {
    row_oplog =
        CreateRowOpLog_(update_size_, sample_row_, dense_row_oplog_capacity_);
...
bool SparseOpLog::FindAndLock(int32_t row_id, OpLogAccessor *oplog_accessor) {
  locks_.Lock(row_id, oplog_accessor->get_unlock_ptr());
  AbstractRowOpLog *row_oplog;
  if (oplog_map_.find(row_id, row_oplog)) {
    oplog_accessor->set_row_oplog(row_oplog);
...
bool SparseOpLog::GetEraseOpLogIf(int32_t row_id, GetOpLogTestFunc test,
                                  void *test_args,
                                  AbstractRowOpLog **row_oplog_ptr) {
  Unlocker<> unlocker;
  locks_.Lock(row_id, &unlocker);
...
SparseOpLog::GetAppendOnlyBuffer(int32_t comm_channel_idx) {
  LOG(FATAL) << "Unsupported operation for OpLogType";
  return 0;
}

void SparseOpLog::PutBackBuffer(int32_t comm_channel_idx,
