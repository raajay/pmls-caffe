...
public:
  AppendOnlyOpLog(size_t append_only_buff_capacity,
    ...
    for (int32_t i = 0; i < GlobalContext::get_num_comm_channels_per_client();
      ...
      oplog_partitions_[i] = new AppendOnlyOpLogPartition(
          ...
          dense_row_oplog_capacity, append_only_per_thread_buff_pool_size);
    }
  }
  ...
  bool GetEraseOpLogIf(int32_t row_id, GetOpLogTestFunc test, void *test_args,
    ...
    int32_t partition_num = GlobalContext::GetPartitionCommChannelIndex(row_id);
    return oplog_partitions_[partition_num]->GetEraseOpLogIf(
        row_id, test, test_args, row_oplog_ptr);
  }
  ...
  AbstractAppendOnlyBuffer *GetAppendOnlyBuffer(int32_t comm_channel_idx) {
    return oplog_partitions_[comm_channel_idx]->GetAppendOnlyBuffer(
        comm_channel_idx);
  }
