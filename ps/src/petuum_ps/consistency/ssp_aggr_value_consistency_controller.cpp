...
void SSPAggrValueConsistencyController::Inc(int32_t row_id, int32_t column_id,
                                            const void *delta) {
  size_t thread_update_count = thread_cache_->IndexUpdateAndGetCount(row_id, 1);
...
void SSPAggrValueConsistencyController::BatchInc(int32_t row_id,
  ...
  if (client_row != 0) {
    ...
    importance =
        sample_row_->GetAccumImportance(column_ids, updates, num_updates);
  }
  meta_row_oplog->GetMeta().accum_importance(importance);
  CheckAndFlushThreadCache(thread_update_count);
}

void SSPAggrValueConsistencyController::DenseBatchInc(int32_t row_id,
                                                      const void *updates,
                                                      int32_t index_st,
                                                      int32_t num_updates) {
  size_t thread_update_count =
  ...
  if (client_row != 0) {
    if (!version_maintain_) {
      ...
      importance = client_row->GetRowDataPtr()->GetDenseAccumImportance(
          updates, index_st, num_updates);
    }
  } else {
    importance =
        sample_row_->GetDenseAccumImportance(updates, index_st, num_updates);
  }
  meta_row_oplog->GetMeta().accum_importance(importance);
  CheckAndFlushThreadCache(thread_update_count);
}
