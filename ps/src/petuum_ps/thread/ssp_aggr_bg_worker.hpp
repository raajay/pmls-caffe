...
public:
  SSPAggrBgWorker(int32_t id, int32_t comm_channel_idx,
      ...
      : SSPPushBgWorker(id, comm_channel_idx, tables, init_barrier,
                        create_table_barrier, system_clock, system_clock_mtx,
                        system_clock_cv, bg_server_clock),
        min_table_staleness_(INT_MAX), oplog_send_milli_sec_(0),
        suppression_level_(0), suppression_level_min_(0), clock_tick_sec_(0),
        suppression_on_(false), early_comm_on_(false) {
...
protected:
  ...
  void ReadTableOpLogsIntoOpLogMeta(int32_t table_id, ClientTable *table);
  ...
  BgOpLogPartition *PrepareOpLogsNormal(int32_t table_id, ClientTable *table,
                                        int32_t clock_to_push);

  BgOpLogPartition *PrepareOpLogsAppendOnly(int32_t table_id,
                                            ClientTable *table,
                                            int32_t clock_to_push);

  void PrepareOpLogsNormalNoReplay(int32_t table_id, ClientTable *table,
                                   int32_t clock_to_push);

  void PrepareOpLogsAppendOnlyNoReplay(int32_t table_id, ClientTable *table,
                                       int32_t clock_to_push);
  ...
  void PrepareBgIdleOpLogsNormalNoReplay(int32_t table_id, ClientTable *table);
