...
public:
  ...
  SerializedOpLogBuffer(bool dense_serialize)
      : size_(0), mem_(new uint8_t[capacity_]), num_row_oplogs_(0) {
    if (dense_serialize) {
  ...
  ~SerializedOpLogBuffer() { delete[] mem_; }
  ...
  const uint8_t *get_mem() const { return mem_; }

  size_t get_num_row_oplogs() const { return num_row_oplogs_; }
...
public:
  RowOpLogSerializer(bool dense_serialize, int32_t my_comm_channel_idx)
      : dense_serialize_(dense_serialize),
        my_comm_channel_idx_(my_comm_channel_idx), total_accum_size_(0),
  ...
  size_t get_total_accum_size() const { return total_accum_size_; }

  size_t get_total_accum_num_rows() const { return total_accum_num_rows_; }
  ...
  size_t AppendRowOpLog(int32_t row_id, AbstractRowOpLog *row_oplog) {
    ...
    if (map_iter == buffer_map_.end()) {
      buffer_map_.insert(
          std::make_pair(server_id, std::vector<SerializedOpLogBuffer *>(1)));
    ...
    if (serialized_size == 0) {
      SerializedOpLogBuffer *new_buffer =
          new SerializedOpLogBuffer(dense_serialize_);
  ...
  void
  GetServerTableSizeMap(std::map<int32_t, size_t> *table_num_bytes_by_server) {
    for (auto &buff_pair : buffer_map_) {
      std::vector<SerializedOpLogBuffer *> &buff_vec = buff_pair.second;
      CHECK(buff_vec.size() > 0);
      size_t &per_server_table_size =
          (*table_num_bytes_by_server)[buff_pair.first];
...
private:
  ...
  std::unordered_map<int32_t, std::vector<SerializedOpLogBuffer *>> buffer_map_;
