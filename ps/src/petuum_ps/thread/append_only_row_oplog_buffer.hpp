...
public:
  AppendOnlyRowOpLogBuffer(int32_t row_oplog_type,
                           const AbstractRow *sample_row, size_t update_size,
                           size_t dense_row_oplog_capacity,
                           AppendOnlyOpLogType append_only_oplog_type)
      : update_size_(update_size), sample_row_(sample_row),
  ...
  void BatchIncTmp(int32_t row_id, const int32_t *col_ids, const void *updates,
                   int32_t num_updates);

  void BatchInc(int32_t row_id, const int32_t *col_ids, const void *updates,
                int32_t num_updates);
...
private:
  void BatchIncGeneral(AbstractRowOpLog *row_oplog, const int32_t *col_ids,
                       const void *updates, int32_t num_updates);

  void BatchIncDense(AbstractRowOpLog *row_oplog, const int32_t *col_ids,
                     const void *updates, int32_t num_updates);

  typedef void (AppendOnlyRowOpLogBuffer::*BatchIncFunc)(
      AbstractRowOpLog *row_oplog, const int32_t *col_ids, const void *updates,
      int32_t num_updates);
