...
Solver<Dtype>::Solver(
    ...
    const int thread_id)
    : net_(), layer_blobs_global_idx_ptr_(layer_blobs_global_idx_ptr),
...
template <typename Dtype> void Solver<Dtype>::InitTrainNet() {
  ...
  net_->set_table(layer_blobs_global_idx_ptr_->at(train_net_output_name)[0]);
...
template <typename Dtype> void Solver<Dtype>::InitTestNets() {
  ...
  for (int i = 0; i < num_test_net_instances; ++i) {
    ...
    if (client_id_ == 0 && thread_id_ == 0) {
      LOG(INFO) << "Creating test net (#" << i << ") specified by "
                << sources[i];
    }
    // set test net id
...
template <typename Dtype> void Solver<Dtype>::InitSVB() {
...
template <typename Dtype> void Solver<Dtype>::Solve(const char *resume_file) {
  ...
  if (thread_id_ == 0) {
    ...
    if (param_.test_interval()) {
      for (int test_net_id = 0; test_net_id < test_nets_.size();
           ++test_net_id) {
        ...
        for (int k = 0; k < result[j]->count(); ++k) {
          if (client_id_ == 0 && thread_id_ == 0) {
            ...
            LOG(INFO) << "    Train net output #"
                      << (score_idx - caffe::kNumFixedCols) << ": "
                      << output_name << " = " << result_vec[k]
                      << loss_msg_stream.str();
          }
        ...
        for (int j = 0; j < layer_params_id.size(); ++j) {
          ...
          if (util::Context::use_svb() &&
            ...
            sync_thread = new std::thread(
                &Solver::ThreadSyncWithSVB, this, net_->params()[param_id],
                param_id, layers[i], i, top_vecs[i], bottom_vecs[i]);
          } else {
            sync_thread = new std::thread(
                &Solver::ThreadSyncWithPS, this, net_->params()[param_id],
                param_id, param_owner, clock_counter_ - param_table_staleness_
#ifdef USE_PS_THIN
                ,
                sync_thread_offset
#endif
...
/// This function is used for created thread to sync one (conv) layer with the
/// PS
template <typename Dtype>
...
template <typename Dtype>
void Solver<Dtype>::ThreadSyncWithSVB(const shared_ptr<Blob<Dtype>> &param,
                                      const int param_id,
                                      const shared_ptr<Layer<Dtype>> &layer,
                                      const int layer_id,
...
template <typename Dtype> void Solver<Dtype>::JoinSyncThreads() {
...
template <typename Dtype> void Solver<Dtype>::Test(const int test_net_id) {
  ...
  const int num_test_iters = (avg + num_threads_ - 1) / num_threads_;
...
template <typename Dtype> void Solver<Dtype>::Restore(const char *state_file) {
  ...
  string s_state_file =
      string(state_file) + string(client_id_str_buffer) + thread_id_str_buffer;
...
void Solver<Dtype>::PrintOutputBlobs(shared_ptr<Net<Dtype>> &net,
  ...
  for (int display_iter = 0; display_iter < num_lines; ++display_iter) {
    ...
    const auto &output_row =
        net->table()->template Get<petuum::DenseRow<Dtype>>(display_iter,
                                                            &row_acc);
    output_row.CopyToVector(&output_cache);
    outfile << output_cache[caffe::kColIdxOutputTableIter] << ","
    ...
    for (int j = 0; j < output_blobs.size(); ++j) {
      for (int k = 0; k < output_blobs[j]->count(); ++k) {
        Dtype result_k =
            output_cache[score_idx] / (num_clients_ * num_threads_);
        outfile << result_k << ",";
        ++score_idx;
      }
    }
...
template <typename Dtype> void SGDSolver<Dtype>::PreSolve() {
  ...
  for (int i = 0; i < net_params.size(); ++i) {
    ...
    history_.push_back(shared_ptr<Blob<Dtype>>(
        new Blob<Dtype>(net_param->num(), net_param->channels(),
                        net_param->height(), net_param->width())));
    update_.push_back(shared_ptr<Blob<Dtype>>(
        new Blob<Dtype>(net_param->num(), net_param->channels(),
                        net_param->height(), net_param->width())));
    temp_.push_back(shared_ptr<Blob<Dtype>>(
        new Blob<Dtype>(net_param->num(), net_param->channels(),
                        net_param->height(), net_param->width())));
  }
}
...
void SGDSolver<Dtype>::ComputeUpdateValue(const int param_id) {
  ...
  case Caffe::CPU:
    ...
    if (local_decay) {
      if (regularization_type == "L2") {
        // add weight decay
        caffe_axpy(net_param->count(), local_decay, net_param->cpu_data(),
                   net_param->mutable_cpu_diff());
      } else if (regularization_type == "L1") {
        caffe_cpu_sign(net_param->count(), net_param->cpu_data(),
                       temp_[param_id]->mutable_cpu_data());
        caffe_axpy(net_param->count(), local_decay, temp_[param_id]->cpu_data(),
                   net_param->mutable_cpu_diff());
      } else {
    ...
    caffe_cpu_axpby(net_param->count(), local_rate, net_param->cpu_diff(),
  ...
  case Caffe::GPU:
    ...
    if (local_decay) {
      if (regularization_type == "L2") {
        // add weight decay
        caffe_gpu_axpy(Caffe::GetDeviceId(), net_param->count(), local_decay,
                       net_param->gpu_data(), net_param->mutable_gpu_diff());
      } else if (regularization_type == "L1") {
        caffe_gpu_sign(net_param->count(), net_param->gpu_data(),
...
template <typename Dtype> void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::GPU:
    ...
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      if (local_decay) {
        if (regularization_type == "L2") {
          ...
          caffe_gpu_axpy(Caffe::GetDeviceId(), net_params[param_id]->count(),
                         local_decay, net_params[param_id]->gpu_data(),
      ...
      caffe_gpu_axpby(Caffe::GetDeviceId(), net_params[param_id]->count(),
                      local_rate, net_params[param_id]->gpu_diff(), momentum,
                      history_[param_id]->mutable_gpu_data());
      // copy
  ...
  case Caffe::GPU:
    ...
    if (local_decay) {
      if (regularization_type == "L2") {
        ...
        caffe_gpu_axpy(Caffe::GetDeviceId(), net_params[param_id]->count(),
                       local_decay, this->temp_[param_id]->gpu_data(),
                       net_params[param_id]->mutable_gpu_diff());
      } else {
        LOG(FATAL) << "Unknown regularization type: " << regularization_type;
      }
    ...
    // compute udpate: step back then over step
    caffe_gpu_axpby(Caffe::GetDeviceId(), net_params[param_id]->count(),
                    Dtype(1) + momentum, this->history_[param_id]->gpu_data(),
                    -momentum, this->update_[param_id]->mutable_gpu_data());
...
template <typename Dtype> void NesterovSolver<Dtype>::ComputeUpdateValue() {
  vector<shared_ptr<Blob<Dtype>>> &net_params = this->net_->params();
  vector<float> &net_params_lr = this->net_->params_lr();
  vector<float> &net_params_weight_decay = this->net_->params_weight_decay();
  // get the learning rate
...
void AdaGradSolver<Dtype>::ComputeUpdateValue(const int param_id) {
  ...
  case Caffe::GPU:
    ...
    // scale and copy
    caffe_gpu_axpby(Caffe::GetDeviceId(), net_params[param_id]->count(),
                    local_rate, this->update_[param_id]->gpu_data(), Dtype(0),
                    net_params[param_id]->mutable_gpu_diff());
#else
    NO_GPU;
#endif
...
template <typename Dtype> void AdaGradSolver<Dtype>::ComputeUpdateValue() {
  vector<shared_ptr<Blob<Dtype>>> &net_params = this->net_->params();
  vector<float> &net_params_lr = this->net_->params_lr();
  vector<float> &net_params_weight_decay = this->net_->params_weight_decay();
  // get the learning rate
