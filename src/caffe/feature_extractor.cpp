...
void FeatureExtractor<Dtype>::ExtractFeatures(const NetParameter &net_param) {
  ...
  for (size_t i = 0; i < num_features; ++i) {
    ...
    leveldb::Status status =
        leveldb::DB::Open(options, leveldb_names[i].c_str(), &db);
  ...
  vector<shared_ptr<leveldb::WriteBatch>> feature_batches(
      num_features, shared_ptr<leveldb::WriteBatch>(new leveldb::WriteBatch()));
  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        feature_blob_data =
            feature_blob->mutable_cpu_data() + feature_blob->offset(n);
