...
void InnerProductLayer<Dtype>::LayerSetUp(
  ...
  if (this->blobs_.size() > 0) {
    ...
    this->blobs_[0].reset(new Blob<Dtype>(
        1, 1, N_, K_, BlobProto_BlobMode_GLOBAL, weight_table_id));
    if (init_ps) {
    ...
    if (bias_term_) {
      ...
      this->blobs_[1].reset(new Blob<Dtype>(
          1, 1, 1, N_, BlobProto_BlobMode_GLOBAL, bias_table_id));
      if (init_ps) {
...
void InnerProductLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
  ...
  M_ = bottom[0]->num();
  CHECK_EQ(bottom[0]->count() / bottom[0]->num(), K_)
      << "Input size "
...
template <typename Dtype>
void
InnerProductLayer<Dtype>::ComputeGradientFromSV_cpu(const SufficientVector *v) {
  // Gradient with respect to weight
