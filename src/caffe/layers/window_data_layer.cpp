...
void WindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype> *> &bottom,
  ...
  LOG(INFO) << "Window data layer:" << std::endl
            ...
            << std::endl << "  background (non-object) overlap threshold: "
            << this->layer_param_.window_data_param().bg_threshold()
            << std::endl << "  foreground sampling fraction: "
...
template <typename Dtype> void WindowDataLayer<Dtype>::InternalThreadEntry() {
        ...
        for (int h = 0; h < cv_cropped_img.rows; ++h) {
          for (int w = 0; w < cv_cropped_img.cols; ++w) {
            ...
            top_data[((item_id * channels + c) * crop_size + h + pad_h) *
                     ...
                     w + pad_w] =
                (pixel -
                 mean[(c * mean_height + h + mean_off + pad_h) * mean_width +
                      w + mean_off + pad_w]) *
                scale;
          }
        }
