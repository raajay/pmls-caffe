...
void SoftmaxWithLossLayer<Dtype>::LayerSetUp(
  ...
  softmax_layer_->SetUp(softmax_bottom_vec_, &softmax_top_vec_, this->net_id_,
                        this->thread_id_, init_ps, num_tables,
                        layer_name_to_blob_global_idx);
}
...
template <typename Dtype>
void
SoftmaxWithLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype> *> &bottom,
                                         vector<Blob<Dtype> *> *top) {
  ...
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < spatial_dim; j++) {
      loss -= log(std::max(
          prob_data[i * dim +
                    static_cast<int>(label[i * spatial_dim + j]) * spatial_dim +
                    j],
          Dtype(FLT_MIN)));
    }
  }
...
template <typename Dtype>
void
SoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype> *> &top,
