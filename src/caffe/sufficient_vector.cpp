...
SufficientVector::SufficientVector(const size_t a_size, const size_t b_size,
                                   const int layer_id) {
...
void SufficientVector::Reshape(const size_t a_size, const size_t b_size) {
  CHECK_GE(a_size, 0);
  CHECK_GE(b_size, 0);
  a_size_ = a_size;
  b_size_ = b_size;
...
template <typename Dtype> void SufficientVector::ToProto(SVProto *proto) const {
