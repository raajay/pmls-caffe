...
Caffe::Caffe() : random_generator_(), mode_(Caffe::CPU), phases_(NULL) {}
...
void Caffe::InitDevices(const std::vector<int> &device_ids,
...
int Caffe::GetDeviceId(const int thread_id) {
  int d = 0;
  if (Caffe::mode() == GPU) {
    CHECK_GT(Get().threads_devices_.size(), 0)
        << "Threads devices mapping is empty";
    CHECK(Get().threads_devices_.count(thread_id) > 0)
        << "Cannot find binded device";
...
void Caffe::InitDevice(const int device_id) {
  for (int i = 0; i < Get().device_ids_.size(); ++i) {
    CHECK(Get().device_ids_[i] != device_id)
        << "Duplicated device id: " << device_id;
  }
  ...
  CURAND_CHECK(curandSetPseudoRandomGeneratorSeed(
      Get().curand_generator_[device_id], cluster_seedgen()));
  SyncDevice();
}

const vector<int> &Caffe::GetActiveDevices() { return Get().device_ids_; }

void Caffe::SyncDevice() { CUDA_CHECK(cudaDeviceSynchronize()); }
