...
// by MsgTracker before sent out (MsgTracker may reject the sending, more
// details
...
// 2) If the message is okay to be sent, properly set its sequence number and
// acknowledgement
// number.
// 3) After a message is sent, keep it until proper acknowledgement is received,
// so that a message may be retransmitted. However, it's up to the client to
// decide
// when to retransmit.

// For a received message, the MsgTracker checks the status of the message and
// decides
...
// 2) out of order -- expected to be received in the future but some message in
// the middle
// is missing;
// 3) duplicated -- the message has been received before, but haven't been
// acknowledged
...
// 2) A message is never retransmitted after an acknowledgement of it or any
// message
...
class MsgTracker {
  ...
  enum MsgStatus {
    ...
    RECV_OUTORDER,  // The received message is valid but it is out of order,
                    // which
                    // indicates a message loss.
    RECV_DUPLICATE, // The received message is a a duplicated message
    RECV_UNBLOCK,   // Due to the received message, some message on hold may be
                    // send out.
    RECV_REPLY,     // The received message is valid and a reply should happen
                    // immediately
    // otherwise the sender might not send any more messages or start
    // sending duplicate messages.
    RECV_ERR // The message should not have been received. Receiving it
             // indicates
             // error.
  };
  ...
  public:
    ...
    uint32_t
        recv_ack_num_; // the highest sequence number that I have acknowledge

    ClientInfo(int32_t circ_msg_buff_capacity,
               int32_t circ_sent_msg_buff_capacity)
        : send_seq_num_(-1), send_ack_num_(-1), recv_seq_num_(-1),
          recv_ack_num_(-1), num_buffered_msgs_(0),
          circ_msg_buff_st_(
              0), // points to the next available slot for insertion
          circ_msg_buff_end_(0), // points to the next slot to read, which is
                                 // not necessarily valid
          circ_msg_buff_capacity_(circ_msg_buff_capacity), // can be used to
                                                           // distingush full
                                                           // vs. empty
          circ_msg_buff_(new MsgBuffer[circ_msg_buff_capacity]),
          num_buffered_sent_msgs_(0),
          circ_sent_msg_buff_st_(
              0), // points to the next available slot for insertion
          circ_sent_msg_buff_end_(0), // points to the next slot to read, which
                                      // is not necessarily valid
          circ_sent_msg_buff_capacity_(
              circ_sent_msg_buff_capacity), // can be used to distingush full
                                            // vs. empty
    ...
    int32_t get_num_buffered_msgs() { return num_buffered_msgs_; }
    ...
    void AddOneSentMsg(MsgBase *new_msg) {
      ...
      circ_sent_msg_buff_st_ =
          (circ_sent_msg_buff_st_ + 1) % circ_sent_msg_buff_capacity_;
    ...
    void *GetOneSentMsg() {
      ...
      circ_sent_msg_buff_end_ =
          (circ_sent_msg_buff_end_ + 1) % circ_sent_msg_buff_capacity_;
      --num_buffered_sent_msgs_;
      return msg;
    }

    int32_t get_num_buffered_sent_msgs() { return num_buffered_sent_msgs_; }

    void ReleasesNSentMsgs(int32_t num_msgs) {
      for (int i = 0; i < num_msgs; ++i) {
        bool should_free;
        void *msg = circ_sent_msg_buff_[circ_sent_msg_buff_end_].ReleaseMsg(
            &should_free);
