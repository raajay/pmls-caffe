...
MsgTracker::MsgTracker(uint32_t window_size) : window_size_(window_size) {}
...
MsgTracker::MsgStatus MsgTracker::PrepareSend(int32_t client_id,
                                              NumberedMsg *msg) {
  ...
  if (iter->second->send_seq_num_ - iter->second->send_ack_num_ ==
      window_size_) {
    ok_to_send = false;
  } else {
    ok_to_send = true;
...
MsgTracker::MsgStatus MsgTracker::CheckGetSendInfo(int32_t client_id,
...
MsgTracker::MsgStatus MsgTracker::CheckRecv(int32_t client_id,
                                            NumberedMsg *msg) {
  ...
  if (msg->get_seq_num() == (iter->second->recv_seq_num_ + 1)) {
    ...
    if ((iter->second->recv_seq_num_ - iter->second->recv_ack_num_) ==
        window_size_) {
      return RECV_REPLY;
    } else {
      return RECV_OK;
    ...
    } else if ((iter->second->recv_ack_num_ - msg->get_seq_num()) <=
               window_size_) {
