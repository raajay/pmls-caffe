...
namespace petuum {
    ...
    Scheduler* MLFabricScheduler::scheduler_;
    ...
    void MLFabricScheduler::Init() {
      ...
      scheduler_  = new PassThroughScheduler();

      int32_t recv_id = GlobalContext::get_scheduler_recv_thread_id();
      scheduler_recv_thread_ = new SchedulerRecvThread(recv_id, &init_barrier_, scheduler_);
      scheduler_recv_thread_->Start();

      int32_t send_id = GlobalContext::get_scheduler_send_thread_id();
      scheduler_send_thread_ = new SchedulerSendThread(send_id, &init_barrier_, scheduler_);
