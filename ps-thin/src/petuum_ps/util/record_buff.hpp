...
public:
  RecordBuff() { LOG(FATAL) << "Default constructor called"; }
  RecordBuff(void *mem, size_t size)
      : mem_(reinterpret_cast<uint8_t *>(mem)), mem_size_(size), offset_(0) {}
  ~RecordBuff() {}

  RecordBuff(RecordBuff &&other)
      : mem_(other.mem_), mem_size_(other.mem_size_),
        offset_(other.offset_){// VLOG(0) << "mem_size_ = " << mem_size_;
        };
  ...
  size_t GetMemUsedSize() { return offset_; }
  ...
  void PrintInfo() const {
    VLOG(0) << "mem_ = " << mem_ << " mem_size_ = " << mem_size_
            << " offset = " << offset_;
  }
