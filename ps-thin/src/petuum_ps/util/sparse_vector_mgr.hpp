...
public:
  ...
  size_t get_size() const { return sparse_vector_->get_size(); }

  uint8_t *GetValPtr(int32_t key) {
    uint8_t *val_ptr = sparse_vector_->GetValPtr(key);
    if (val_ptr == 0) {
      capacity_ *= 2;
      SparseVector *new_sparse_vector =
          new SparseVector(capacity_, value_size_);
  ...
  void ResetVal() { sparse_vector_->Reset(); }
  ...
  uint8_t *get_data_ptr() { return sparse_vector_->get_data_ptr(); }
