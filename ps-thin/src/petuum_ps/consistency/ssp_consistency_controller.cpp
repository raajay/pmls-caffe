...
namespace petuum {
    ...
    ClientRow *SSPConsistencyController::Get(int32_t row_id, RowAccessor *row_accessor, int32_t clock) {
    ...
    void SSPConsistencyController::BatchInc(int32_t row_id,
        ...
        VLOG(20) << "Updating row=" << row_id << " for table=" << table_id_ << " with model version=" << global_version;

        // update the thread index saying that row id is updated
        // thread_cache_->IndexUpdate(row_id);

        // (raajay) In Caffe, only a single thread is responsible for updating and reading from a single table. This,
        // provides an opportunity to directly write to process level data structures rather than having a
        // thread level cache.
        oplog_index_.AddRowIndex(row_id);
    ...
    void SSPConsistencyController::DenseBatchInc(int32_t row_id,
        ...
        // TODO (raajay): Remove locking for writing to table oplog for Caffe specifically?
        OpLogAccessor oplog_accessor;
        ...
        if (new_create) {
            ...
            const auto *deltas_uint8 = reinterpret_cast<const uint8_t *>(updates);
            (this->*DenseBatchIncOpLog_)(&oplog_accessor, deltas_uint8, index_st, num_updates);
        }
        STATS_APP_SAMPLE_BATCH_INC_OPLOG_END();
    ...
    void SSPConsistencyController::DenseBatchIncDenseOpLog(OpLogAccessor *oplog_accessor,
        ...
        auto *oplog_delta = reinterpret_cast<uint8_t *>(oplog_accessor->get_row_oplog()->FindCreate(index_st));
        for (int i = 0; i < num_updates; ++i) {
...
//        thread_cache_->FlushCache(process_storage_, oplog_, sample_row_);
    ...
    void SSPConsistencyController::Clock() {
        // (raajay): Removed the code to flush thread level values (index, oplog) into process level data structures,
