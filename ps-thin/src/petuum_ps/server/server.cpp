...
namespace petuum {
  ...
  void Server::AddRowRequest(int32_t bg_id,
    ...
    if (clock_bg_row_requests_[clock].count(bg_id) == 0) {
      clock_bg_row_requests_[clock]
          .insert(std::make_pair(bg_id,
  ...
  void Server::ApplyOpLogUpdateVersion(const void *oplog,
    ...
    *observed_delay = -1; // init the observed delay
    ...
    const void *updates = oplog_reader.Next(&table_id, &row_id,
            &update_model_version, &column_ids, &num_updates, &started_new_table);
