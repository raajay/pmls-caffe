...
namespace petuum {
    ...
    public:
        ...
        size_t Init(int32_t server_id, const TwoDimCounter &server_table_byte_counter) {
            ...
            for (auto table_id : non_empty_tables) {
                ...
                // 1) 4 bytes for table_id,
                // 2) 4 bytes for update size (size of row) in this table
                // 3) Sum of all row oplogs + num rows from current table to server
                total_size += sizeof(int32_t) + sizeof(int32_t)
        ...
        /**
         * Return the offset into the memory where all table_id relevant data will be stored.
         * @param table_id
         * @return
         */
