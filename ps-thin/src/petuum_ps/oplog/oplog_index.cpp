...
PartitionOpLogIndex::PartitionOpLogIndex(size_t capacity):
    ...
    shared_oplog_index_(new cuckoohash_map<int32_t, bool> (static_cast<size_t>(capacity * kCuckooExpansionFactor))){
}
...
void PartitionOpLogIndex::AddIndex(const std::unordered_set<int32_t> &oplog_index) {
  smtx_.lock_shared();
...
cuckoohash_map<int32_t, bool> *PartitionOpLogIndex::Reset() {
  ...
  shared_oplog_index_ = new cuckoohash_map<int32_t, bool> (static_cast<size_t>(capacity_ * kCuckooExpansionFactor));
  smtx_.unlock();
  return old_index;
}
...
void TableOpLogIndex::AddIndex(int32_t partition_num, const std::unordered_set<int32_t> &oplog_index) {
  partition_oplog_index_[partition_num].AddIndex(oplog_index);
}

cuckoohash_map<int32_t, bool> *TableOpLogIndex::ResetPartition(int32_t partition_num) {
...
size_t TableOpLogIndex::GetNumRowOpLogs(int32_t partition_num) {
  return partition_oplog_index_[partition_num].GetNumRowOpLogs();
     ...
     * Thus, the function is guaranteed to be called from a single thread exclusively.
     * @param row_id
     */
