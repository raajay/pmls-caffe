...
bool IncAppendOnlyBuffer::Inc(int32_t row_id, int32_t col_id,
                              const void *delta) {
  if (size_ + sizeof(int32_t) + sizeof(int32_t) + update_size_ > capacity_)
    return false;
  ...
  for (int i = 0; i < num_updates; ++i) {
    ...
    *(reinterpret_cast<int32_t *>(buff_.get() + size_)) = col_ids[i];
    ...
    memcpy(buff_.get() + size_,
           reinterpret_cast<const uint8_t *>(deltas) + update_size_ * i,
           update_size_);
    size_ += update_size_;
  }
  return true;
...
void IncAppendOnlyBuffer::InitRead() { read_ptr_ = buff_.get(); }

const void *IncAppendOnlyBuffer::Next(int32_t *row_id, int32_t const **col_ids,
                                      int32_t *num_updates) {
