...
#include <petuum_ps/util/sparse_vector_mgr.hpp>
...
public:
  SparseVectorRowOpLog(size_t init_capacity, InitUpdateFunc InitUpdate,
                       CheckZeroUpdateFunc CheckZeroUpdate, size_t update_size)
      : AbstractRowOpLog(update_size), oplogs_(init_capacity, update_size),
        InitUpdate_(InitUpdate), CheckZeroUpdate_(CheckZeroUpdate) {
  ...
  void *Find(int32_t col_id) { return oplogs_.GetValPtr(col_id); }

  const void *FindConst(int32_t col_id) const {
    return oplogs_.GetValPtrConst(col_id);
  }

  void *FindCreate(int32_t col_id) { return oplogs_.GetValPtr(col_id); }
  ...
  size_t GetSize() const { return oplogs_.get_size(); }
