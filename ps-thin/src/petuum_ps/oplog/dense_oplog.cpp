...
DenseOpLog::DenseOpLog(int capacity, const AbstractRow *sample_row,
    ...
    : update_size_(sample_row->get_update_size()),
      ...
      dense_row_oplog_capacity_(dense_row_oplog_capacity), capacity_(capacity) {
...
bool DenseOpLog::FindAndLock(int32_t row_id, OpLogAccessor *oplog_accessor) {
  locks_.Lock(row_id, oplog_accessor->get_unlock_ptr());
...
bool DenseOpLog::GetEraseOpLogIf(int32_t row_id, GetOpLogTestFunc test,
                                 void *test_args,
                                 AbstractRowOpLog **row_oplog_ptr) {
  Unlocker<> unlocker;
  locks_.Lock(row_id, &unlocker);
...
DenseOpLog::GetAppendOnlyBuffer(int32_t comm_channel_idx) {
  LOG(FATAL) << "Unsupported operation for OpLogType";
  return 0;
}

void DenseOpLog::PutBackBuffer(int32_t comm_channel_idx,
