...
template <typename V> MultiplicativeDenseRow<V>::MultiplicativeDenseRow() {}

template <typename V> MultiplicativeDenseRow<V>::~MultiplicativeDenseRow() {}

template <typename V> void MultiplicativeDenseRow<V>::Init(int32_t capacity) {
...
template <typename V> AbstractRow *MultiplicativeDenseRow<V>::Clone() const {
...
template <typename V> size_t MultiplicativeDenseRow<V>::SerializedSize() const {
...
template <typename V>
bool MultiplicativeDenseRow<V>::Deserialize(const void *data,
                                            size_t num_bytes) {
...
template <typename V>
void MultiplicativeDenseRow<V>::ApplyInc(int32_t column_id,
                                         const void *update) {
...
template <typename V>
void MultiplicativeDenseRow<V>::ApplyIncUnsafe(int32_t column_id,
                                               const void *update) {
  assert(column_id < (int32_t)data_.size());
  data_[column_id] *= *(reinterpret_cast<const V *>(update));
}
...
template <typename V>
void MultiplicativeDenseRow<V>::SubtractUpdates(int32_t column_id,
                                                void *update1,
                                                const void *update2) const {
  *(reinterpret_cast<V *>(update1)) -= *(reinterpret_cast<const V *>(update2));
}
...
template <typename V>
void MultiplicativeDenseRow<V>::InitUpdate(int32_t column_id,
                                           void *update) const {
...
template <typename V> int32_t MultiplicativeDenseRow<V>::get_capacity() {
  return capacity_;
}
