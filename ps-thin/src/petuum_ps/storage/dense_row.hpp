...
template <typename V> DenseRow<V>::DenseRow() {}

template <typename V> DenseRow<V>::~DenseRow() {}

template <typename V> void DenseRow<V>::Init(int32_t capacity) {
...
template <typename V> AbstractRow *DenseRow<V>::Clone() const {
...
template <typename V> size_t DenseRow<V>::SerializedSize() const {
  return data_.size() * sizeof(V);
}

template <typename V> size_t DenseRow<V>::Serialize(void *bytes) const {
...
template <typename V> void DenseRow<V>::GetWriteLock() { mtx_.lock(); }

template <typename V> void DenseRow<V>::ReleaseWriteLock() { mtx_.unlock(); }
...
double DenseRow<V>::ApplyIncUnsafeGetImportance(int32_t column_id,
  ...
  double importance =
      (double(data_[column_id]) == 0)
          ? double(type_update * scale)
          : double(type_update * scale) / double(data_[column_id]);
...
double DenseRow<V>::ApplyBatchIncUnsafeGetImportance(const int32_t *column_ids,
  ...
  for (i = 0; i < num_updates; ++i) {
    double importance =
        (double(data_[column_ids[i]]) == 0)
            ? double(update_array[i] * scale)
            : double(update_array[i] * scale) / double(data_[column_ids[i]]);
    data_[column_ids[i]] += (update_array[i] * scale);
...
double DenseRow<V>::ApplyDenseBatchIncUnsafeGetImportance(
  ...
  for (i = 0; i < num_updates; ++i) {
    ...
    double importance =
        (double(data_[col_id]) == 0)
            ? double(update_array[i] * scale)
            : double(update_array[i] * scale) / double(data_[col_id]);
...
template <typename V>
double DenseRow<V>::ApplyDenseBatchIncGetImportance(const void *update_batch,
                                                    int32_t index_st,
                                                    int32_t num_updates,
                                                    double scale) {
  std::unique_lock<std::mutex> lock(mtx_);
...
template <typename V>
void DenseRow<V>::ApplyDenseBatchInc(const void *update_batch, int32_t index_st,
                                     int32_t num_updates, double scale) {
  std::unique_lock<std::mutex> lock(mtx_);
  return ApplyDenseBatchIncUnsafe(update_batch, index_st, num_updates, scale);
}

template <typename V> V DenseRow<V>::operator[](int32_t column_id) const {
  std::unique_lock<std::mutex> lock(mtx_);
  V v = data_[column_id];
  return v;
}

template <typename V> int32_t DenseRow<V>::get_capacity() { return capacity_; }

template <typename V> void DenseRow<V>::CopyToVector(std::vector<V> *to) const {
  std::unique_lock<std::mutex> lock(mtx_);
