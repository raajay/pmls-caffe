...
template <typename V>
class SortedVectorMapRow : public NumericContainerRow<V>,
...
template <typename V> const int32_t SortedVectorMapRow<V>::K_BLOCK_SIZE_ = 32;

template <typename V>
SortedVectorMapRow<V>::SortedVectorMapRow()
    : num_entries_(0), capacity_(0) {}

template <typename V> int32_t SortedVectorMapRow<V>::num_entries() const {
...
template <typename V> void SortedVectorMapRow<V>::Init(int32_t capacity) {
  capacity_ = capacity;
  num_entries_ = 0;
  entries_.reset(new Entry<V>[capacity_]);
}

template <typename V> AbstractRow *SortedVectorMapRow<V>::Clone() const {
...
template <typename V> size_t SortedVectorMapRow<V>::get_update_size() const {
  return sizeof(V);
}

template <typename V> size_t SortedVectorMapRow<V>::SerializedSize() const {
...
template <typename V> void SortedVectorMapRow<V>::GetWriteLock() {
  rw_mutex_.lock();
}

template <typename V> void SortedVectorMapRow<V>::ReleaseWriteLock() {
  rw_mutex_.unlock();
}
...
void SortedVectorMapRow<V>::ApplyBatchIncUnsafe(const int32_t *column_ids,
  ...
  // Global sort.
  std::sort(
      entries_.get(), entries_.get() + num_entries_,
      [](const Entry<V> &i, const Entry<V> &j) { return i.second > j.second; });
}
...
double SortedVectorMapRow<V>::ApplyIncUnsafeGetImportance(int32_t column_id,
  ...
  if (vector_idx != -1) {
    importance = std::abs(
        (double(entries_[vector_idx].second) == 0)
            ? double(typed_update)
...
template <typename V>
void SortedVectorMapRow<V>::ApplyDenseBatchInc(const void *update_batch,
                                               int32_t index_st,
                                               int32_t num_updates) {
  LOG(FATAL) << "Not available";
}
...
template <typename V>
void SortedVectorMapRow<V>::ApplyDenseBatchIncUnsafe(const void *update_batch,
                                                     int32_t index_st,
                                                     int32_t num_updates) {
  const V *typed_updates = reinterpret_cast<const V *>(update_batch);
...
template <typename V> V SortedVectorMapRow<V>::const_iterator::operator*() {
  CHECK(!is_end());
...
typename SortedVectorMapRow<V>::const_iterator::iter_t
    SortedVectorMapRow<V>::const_iterator::
    operator->() {
  CHECK(!is_end());
...
template <typename V> bool SortedVectorMapRow<V>::const_iterator::is_begin() {
  return (curr_ == 0);
}

template <typename V> bool SortedVectorMapRow<V>::const_iterator::is_end() {
