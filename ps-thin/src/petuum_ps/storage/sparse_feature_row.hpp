...
#include <petuum_ps/storage/entry.hpp>
...
public: // AbstractRow implementation.
  ...
  void ApplyBatchInc(const int32_t *column_ids, const void *updates,
                     int32_t num_updates);
...
template <typename V> int32_t SparseFeatureRow<V>::GetNumEntries() const {
...
template <typename V> void SparseFeatureRow<V>::Init(int32_t feature_dim) {
  this->feature_dim_ = feature_dim;
}

template <typename V> AbstractRow *SparseFeatureRow<V>::Clone() const {
...
template <typename V> size_t SparseFeatureRow<V>::get_update_size() const {
  return sizeof(V);
}

template <typename V> size_t SparseFeatureRow<V>::SerializedSize() const {
  // Add feature_dim_ at the beginning.
  return sizeof(int32_t) + this->num_entries_ * sizeof(Entry<V>);
}

template <typename V> size_t SparseFeatureRow<V>::Serialize(void *bytes) const {
...
template <typename V> void SparseFeatureRow<V>::GetWriteLock() {
  rw_mutex_.lock();
}

template <typename V> void SparseFeatureRow<V>::ReleaseWriteLock() {
  rw_mutex_.unlock();
}
...
void SparseFeatureRow<V>::ApplyBatchIncUnsafe(const int32_t *column_ids,
  ...
  for (int i = 0; i < num_updates; ++i) {
    ...
    // Add a new entry.
    if (this->num_entries_ == this->capacity_) {
      ml::SparseFeature<V>::ResetCapacity(this->capacity_ +
                                          this->K_BLOCK_SIZE_);
    }
  ...
  // Global sort.
  std::sort(
      this->entries_.get(), this->entries_.get() + this->num_entries_,
      [](const Entry<V> &i, const Entry<V> &j) { return i.first < j.first; });
}
...
double SparseFeatureRow<V>::ApplyIncUnsafeGetImportance(int32_t column_id,
  ...
  V typed_update = *(reinterpret_cast<const V *>(update));
  if (vector_idx != -1) {
    importance = std::abs((double(this->entries_[vector_idx].second) == 0)
                              ? double(typed_update)
                              : (double(typed_update) /
                                 double(this->entries_[vector_idx].second)));
...
template <typename V>
void SparseFeatureRow<V>::ApplyDenseBatchInc(const void *update_batch,
                                             int32_t index_st,
                                             int32_t num_updates) {
  LOG(FATAL) << "Not available";
}
...
template <typename V>
void SparseFeatureRow<V>::ApplyDenseBatchIncUnsafe(const void *update_batch,
                                                   int32_t index_st,
                                                   int32_t num_updates) {
  const V *typed_updates = reinterpret_cast<const V *>(update_batch);
...
template <typename V> V SparseFeatureRow<V>::const_iterator::operator*() {
  CHECK(!is_end());
...
typename SparseFeatureRow<V>::const_iterator::iter_t
    SparseFeatureRow<V>::const_iterator::
    operator->() {
  CHECK(!is_end());
...
template <typename V> bool SparseFeatureRow<V>::const_iterator::is_begin() {
  return (curr_ == 0);
}

template <typename V> bool SparseFeatureRow<V>::const_iterator::is_end() {
...
template <typename V>
typename SparseFeatureRow<V>::const_iterator SparseFeatureRow<V>::cend() const {
