...
void ReadDataLabelBinary(const std::string &filename, int32_t feature_dim,
                         int32_t num_data,
                         std::vector<AbstractFeature<float> *> *features,
                         std::vector<int32_t> *labels, bool feature_one_based,
  ...
  for (int i = 0; i < num_data; ++i) {
    ...
    // Read the feature
    is.read(reinterpret_cast<char *>(cache.data()),
...
int32_t ParseLibSVMLine(const std::string &line,
  ...
  while (isspace(*ptr) && ptr - line.data() < line.size())
    ++ptr;
  int nnz = 0;
  ...
  std::copy(feature_ids_cache->begin(), feature_ids_cache->begin() + nnz,
            feature_ids->begin());
  std::copy(feature_vals_cache->begin(), feature_vals_cache->begin() + nnz,
            feature_vals->begin());
  return label;
}
...
void ReadDataLabelLibSVM(const std::string &filename, int32_t feature_dim,
                         ...
                         std::vector<AbstractFeature<float> *> *features,
                         std::vector<int32_t> *labels, bool feature_one_based,
                         bool label_one_based, bool snappy_compressed) {
  petuum::HighResolutionTimer read_timer;
  features->resize(num_data);
  labels->resize(num_data);
  std::string file_str = snappy_compressed ? SnappyOpenFileToString(filename)
  ...
  for (std::string line; std::getline(data_stream, line) && i < num_data; ++i) {

    std::vector<int32_t> feature_ids;
    std::vector<float> feature_vals;
    int32_t label = ParseLibSVMLine(line, &feature_ids, &feature_vals,
                                    feature_one_based, label_one_based,
                                    &feature_ids_cache, &feature_vals_cache);

    (*labels)[i] = label;
    (*features)[i] =
        new SparseFeature<float>(feature_ids, feature_vals, feature_dim);
  }
  CHECK_EQ(num_data, i) << "Request to read " << num_data
                        << " data instances but only " << i << " found in "
                        << filename;
...
void ReadDataLabelSparseFeatureBinary(
    const std::string &filename, int32_t feature_dim, int32_t num_data,
    std::vector<AbstractFeature<float> *> *features,
    std::vector<int32_t> *labels, bool feature_one_based, bool label_one_based,
