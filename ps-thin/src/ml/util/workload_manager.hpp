...
public:
  WorkloadManager(const WorkloadManagerConfig &config) {
    ...
    if (global_data) {
      ...
      num_data_per_thread =
          (num_data + num_total_threads - 1) / num_total_threads;

      size_t total_data_assigned = num_data_per_thread * num_total_threads;
      int32_t cutoff_id =
          num_total_threads - 1 - (total_data_assigned - num_data);
    ...
    batch_size_ =
        std::ceil(static_cast<float>(data_idx_end_ - data_idx_begin_) /
  ...
  void Restart() { num_data_this_epoch_ = 0; }
  ...
  bool IsEnd() const { return num_data_this_epoch_ == num_data_per_epoch_; }

  bool IsEndOfBatch() const { return num_data_this_epoch_ % batch_size_ == 0; }
