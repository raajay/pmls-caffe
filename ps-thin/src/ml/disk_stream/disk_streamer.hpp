...
template <typename DATUM> class DiskStreamer {
public:
  // DiskStreamer takes ownership of parser.
  DiskStreamer(const DiskStreamerConfig &config, AbstractParser<DATUM> *parser);
...
DiskStreamer<DATUM>::DiskStreamer(const DiskStreamerConfig &config,
    ...
    : multi_buffer_(config.num_buffers),
      disk_reader_(config.disk_reader_config, &multi_buffer_), parser_(parser),
      barrier_(config.num_worker_threads) {
  CHECK_NE(-1, config.num_worker_threads);
  CHECK(parser_) << "Data parser cannot be null.";
  disk_reader_thread_ = std::thread(&DiskReader::Start, std::ref(disk_reader_));
...
template <typename DATUM> DiskStreamer<DATUM>::~DiskStreamer() {
...
std::vector<DATUM *> DiskStreamer<DATUM>::ParseData(int num_data) {
  ...
  for (int i = 0; i < num_data && curr_work_buffer_->HasMore(); ++i) {
    int num_bytes = 0;
...
template <typename DATUM> void DiskStreamer<DATUM>::Shutdown() {
