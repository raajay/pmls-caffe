...
template <typename V> class DenseFeature : public AbstractFeature<V> {
public:
  ...
  void Init(const std::vector<V> &val) { data_ = val; }

  // Makes it behave like std::vector.
  virtual inline V &operator[](int32_t feature_id) { return data_[feature_id]; }

  virtual inline const std::vector<V> &GetVector() const { return data_; }

  virtual inline std::vector<V> &GetVector() { return data_; }
...
public: // AbstractFeature override.
  ...
  virtual inline int32_t GetNumEntries() const { return data_.size(); }

  // idx is the same as feature id in DenseFeature.
  virtual inline int32_t GetFeatureId(int32_t idx) const { return idx; }

  virtual inline V GetFeatureVal(int32_t idx) const { return data_[idx]; }
